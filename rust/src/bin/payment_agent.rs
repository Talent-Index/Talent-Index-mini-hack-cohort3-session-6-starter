...
//! and log the outcome, approved or not. Wrapped in four required
//! safety pillars: pre-flight checks, a spending limit, an idempotency
//! check, and an audit log entry for every decision.
//!
...
struct Invoice {
    ...
    supplier: &'static str,
    amount_usdc: f64,
    days_overdue: i64, // fixed for this demo, in a real system compute from a real due date
    paid: bool,
...
fn find_overdue_invoices() -> Vec<Invoice> {
...
fn preflight_checks(inv: &Invoice, sent_payments_log: &HashSet<String>) -> Vec<String> {
...
fn erc20_transfer_data(recipient: Address, amount: U256) -> Vec<u8> {
...
async fn send_payment(inv: &Invoice) -> Result<String, Box<dyn std::error::Error>> {
...
fn log_decision(invoice_id: &str, reasoning: &str, approved: bool, tx_hash: Option<&str>) {
